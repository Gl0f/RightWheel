...
const API_BASE_URL = window.location.port === '5500' || window.location.port === '5501'
    ? 'http://127.0.0.1:5000'  // Якщо ми на Live Server -> йдемо на порт Python
    : '';                       // Якщо ми на Render або відкрили через Flask -> йдемо відносно себе
...
async function renderStatistics() {
    ...
    try {
        const response = await fetch('/api/car-of-the-day');
        if (!response.ok) {
...
async function renderFeaturedCars() {
    ...
    elements.featuredCars.innerHTML = '<p>Завантаження каталогу...</p>';

    try {
        // 1. Звертаємось до КАТАЛОГУ (Trims)
        const response = await fetch('/api/trims/latest');
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const latestTrims = await response.json();

        elements.featuredCars.innerHTML = '';

        if (!latestTrims || latestTrims.length === 0) {
            elements.featuredCars.innerHTML = '<p>Каталог поки що порожній.</p>';
            return;
        }
        ...
        latestTrims.forEach(car => {
            ...
            // Зберігаємо дані для URL
            card.dataset.id = car.id;
            card.dataset.brandId = car.brand_id;
            card.dataset.brandName = car.make_name; // В API trims це make_name
            card.dataset.modelId = car.model_id;
            card.dataset.modelName = car.model_name;
            card.dataset.generationId = car.generation_id;
            card.dataset.generationName = car.generation_name || '';

            // Картинка
            const imgUrl = car.image_url || 'https://via.placeholder.com/100x70?text=No+Photo';
            
            // Ціна (якщо вказана офіційна ціна в доларах)
            let priceHtml = '';
            if (car.price_usd) {
                const price = new Intl.NumberFormat('en-US').format(car.price_usd);
                priceHtml = `<span style="color: #38A169; font-size: 15px;">$${price}</span>`;
            }

            card.innerHTML = `
                <div class="featured-car-image">
                    <img src="${imgUrl}" alt="${car.make_name} ${car.model_name}">
                </div>
                <div class="featured-car-content">
                    ...
                    <div class="spec-line">${car.year} • ${car.name}</div>
                    <div class="spec-line">${car.power_hp ? car.power_hp + ' к.с.' : ''} • ${car.fuel || ''}</div>
                    <div class="body-drive-line">${car.body_type || ''} • ${car.drive || ''}</div>
                </div>
            `;

            // Клік веде на ЕНЦИКЛОПЕДІЮ (car.html)
        ...
        elements.featuredCars.innerHTML = `<p style="color: red;">Помилка завантаження даних.</p>`;
    }
}
...
async function openComparisonModal() {
    ...
    try {
        ...
        const fetchPromises = state.comparisonList.map(trimId =>
            fetch(`/api/trims/${trimId}`).then(response => {
...
async function handleGoogleCredentialResponse(response) {
    ...
    try {
        // ДОДАНО: ${API_BASE_URL} перед шляхом
